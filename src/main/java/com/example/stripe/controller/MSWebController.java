...
public class MSWebController {
	...
	@GetMapping("/timetable")
	  String getTimetable(@ModelAttribute DaysOfMonthDTO daysOfMonth, BindingResult errors, final ModelMap model) {
		return timetable( daysOfMonth, errors, model);
	  }
	...
	private String timetable(DaysOfMonthDTO daysOfMonth, BindingResult errors, final ModelMap model) {
		DaysOfMonthDTO days;
		
		//String accessToken = applicationOAuthToken.getAccessToken();
		//String tokenType = applicationOAuthToken.getTokenType();
		//String json = listEventsByOrganisation(ORGANISATION_ID, accessToken, tokenType);
		//JSONObject parser = JSONObject.fromObject(json);
		//eventId: 322882689817
		//seriesId: 199412888317
		//String series = retrieveAnEventSeries("199412888317", accessToken, tokenType);
		...
		if(daysOfMonth.isEmpty()) {
			...
			//String parentEventSeries = retrieveAnEventSeries(FAJR_SERIES_PARENT_ID,  accessToken,  tokenType);
			//Map<String,Object> parentEventSeriesObj = parseMap(parentEventSeries);
			//Map<String,Object> descriptionObj = (Map<String, Object>) parentEventSeriesObj.get("description");
			...
			for (DayOfMonth day: persistDays) {
				...
				if(existingDayOfMonth != null) {
					//update event api
					String eventStartDateTimeUTC = DateUtil.toEventUTCFormattedDate(day.getYear(), day.getMonth(), day.getDay(), day.getHourFajr(), day.getMinuteFajr());
					String eventEndDateTimeUTC = DateUtil.toEventUTCFormattedDatePlusMinutes(day.getYear(), day.getMonth(), day.getDay(), day.getHourFajr(), day.getMinuteFajr(), 20l);
					//String fajrEventUpdated = updateAnEvent(existingDayOfMonth.getObjectId().toString(), eventStartDateTimeUTC, eventEndDateTimeUTC,  accessToken,  tokenType);
					try {
					...
					String fajrUTCTime = DateUtil.toScheduleUTCFormattedDate(day.getYear(), day.getMonth(), day.getDay(), day.getHourFajr(), day.getMinuteFajr());
					//String fajrEventSchedule = createAnEventSchedule(FAJR_SERIES_PARENT_ID, fajrUTCTime, accessToken, tokenType);
					try {
						//Map<String,Object> fajrEventObj = parseMap(fajrEventSchedule);
						//List<Object> fajrEventIds = (List<Object>) fajrEventObj.get("created_event_ids");
